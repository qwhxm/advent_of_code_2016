...
-- | IPv7 addresses given as puzzle input
addresses :: [String]
addresses =
...
-- | parsed IPv7 address, separated into strings outside and inside brackets
type Address = ([String], [String])

-- | parses the given IPv7 address
parse :: String -> Address
parse s =
    let
        ss = splitOneOf ['[', ']'] s
        -- parts of address outside brackets
        o = map fst $ filter (even . snd) $ zip ss [0 ..]
        -- parts of address inside brackets
        i = map fst $ filter (odd . snd) $ zip ss [0 ..]
    in
        (o, i)

-- | returns all four-character substrings of the given string
fourCharacterSubstrings :: String -> [String]
fourCharacterSubstrings s = [take 4 $ drop n s | n <- [0 .. length s - 4]]

-- | determines whether the given four-character sequence is an ABBA
isABBA :: String -> Bool
isABBA [c1, c2, c3, c4] = c1 == c4 && c2 == c3 && c1 /= c2

-- | determines whether the given string contains an ABBA
containsABBA :: String -> Bool
containsABBA s = any isABBA $ fourCharacterSubstrings s

-- | determines whether the given IPv7 address supports transport-layer snooping
supportsTLS :: Address -> Bool
supportsTLS (o, i) = any containsABBA o && not (any containsABBA i)

-- | solution to part one of the puzzle
solution1 =
    let
        -- parsed addresses
        as = map parse addresses
        -- addresses that support TLS
        as' = filter supportsTLS as
    in
        length as'

-- | returns all three-character substrings of the given string
threeCharacterSubstrings :: String -> [String]
threeCharacterSubstrings s = [take 3 $ drop n s | n <- [0 .. length s - 3]]

-- | determines whether the given three-character sequence is an ABA
isABA :: String -> Bool
isABA [c1, c2, c3] = c1 == c3 && c1 /= c2

-- | returns a list of all ABAs in the given string
containedABAs :: String -> [String]
containedABAs s = filter isABA $ threeCharacterSubstrings s

-- | returns BAB corresponding to the given ABA
correspondingBAB :: String -> String
correspondingBAB [c1, c2, c3] | c1 == c3 = [c2, c1, c2]

-- | determines whether the given IPv7 address supports super-secret listening
supportsSSL :: Address -> Bool
supportsSSL (o, i) =
    let
        -- all ABAs contained in parts of address outside brackets
        abas = nub $ concatMap containedABAs o
        -- BABs corresponding to the found ABAs
        babs = map correspondingBAB abas
    in
        any (\bab -> any (isInfixOf bab) i) babs
...
solution2 =
    let
        -- parsed addresses
        as = map parse addresses
        -- addresses that support SSL
        as' = filter supportsSSL as
    in
        length as'
