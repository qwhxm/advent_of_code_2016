...
-- NOTE requires the package "split"
import Data.Char
import Data.List
import Data.List.Split

-- | parsed room specification: encrypted room name, sector ID, and checksum
type Room = (String, Int, String)

-- | room specifications given as puzzle input
rooms :: [String]
rooms =
    ["vxupkizork-sgmtkzoi-pkrrehkgt-zxgototm-644[kotgr]",
     ...
     "dmbttjgjfe-fhh-ufdiopmphz-129[nbqyz]", "tinnm-tzcksf-obozmgwg-194[mduaf]",
     ...
     "avb-abpfdk-185[cwyms]", "xfbqpojafe-kfmmzcfbo-vtfs-uftujoh-857[nserj]",
     ...
     "xmtjbzidx-wpiit-hvivbzhzio-759[kgzsj]", "mvkccspson-nio-ckvoc-666[mnklu]",
     ...
     "zlilocri-oxyyfq-pxibp-939[ilopx]", "xqvwdeoh-hjj-uhvhdufk-985[uixsv]",
     ...
     "kzeed-jll-fsfqdxnx-697[abodg]", "xtwtelcj-rclop-prr-hzcvdsza-977[crlpt]",
     ...
     "mbiyqoxsm-nio-nozvyiwoxd-614[mstez]", "ohmnuvfy-yaa-uhufsmcm-214[muafh]",
     ...
     "bqvvu-ywjzu-paydjkhkcu-810[rgpzu]", "cvabijtm-lgm-zmamizkp-954[tsrpy]",
     ...
     "myvybpev-lkcuod-nocsqx-770[yzjqg]", "upq-tfdsfu-cbtlfu-eftjho-623[ftubc]",
     ...
     "ydjuhdqjyedqb-vbemuh-tuiywd-894[oybua]", "dwbcjkun-mhn-bjunb-121[srjlk]",
     ...
     "xcjxjgvoz-vivgtndn-629[yubnv]", "ixeumktoi-lruckx-sgtgmksktz-332[mrldq]",
     ...
     "mfklstdw-jsttal-ksdwk-944[autpj]", "jvsvymbs-jhukf-zlycpjlz-409[borjv]",
     ...
     "pbeebfvir-enoovg-fuvccvat-663[fpume]", "dwbcjkun-mhn-mnbrpw-173[wscyp]",
     "dpmpsgvm-sbccju-nbobhfnfou-987[nmzgw]",
     "eqnqthwn-ecpfa-eqcvkpi-gpikpggtkpi-986[pegik]",
     "npmhcargjc-aylbw-bcnjmwkclr-288[xfyca]", "qfkkj-mfyyj-dezclrp-301[pdsrv]",
     ...
     "ryexqpqhteki-uww-iqbui-972[whlao]", "guahyncw-zfiqyl-mylpcwym-292[hakyd]",
     ...
     "fmsledevhsyw-fewoix-asvowlst-282[sewfl]", "qfkkj-ojp-cpdplcns-197[yovpm]",
     ...
     "ugdgjxmd-vqw-vwhsjlewfl-684[sxfvh]", "ynssr-xzz-xgzbgxxkbgz-839[xzgbs]",
     ...
     "rgllk-ngzzk-emxqe-794[snuev]", "qvbmzvibqwvit-kivlg-zmkmqdqvo-876[ivunt]",
     ...
     "tipfxvezt-upv-kvtyefcfxp-685[buszt]", "apwmeclga-cee-qcptgacq-392[yszmf]",
     ...
     "drxevkzt-sleep-uvgcfpdvek-945[bvnau]", "nzcczdtgp-mfyyj-dlwpd-613[yzbdk]",
     ...
     "qzlozfhmf-dff-zbpthrhshnm-651[mkbun]", "xmrrq-wyy-dgyaklauk-138[yakrd]",
     ...
     "yrwxefpi-hci-xiglrspskc-438[zytvr]", "ujoon-tvv-hwxeexcv-427[ukymv]",
     ...
     "qfkkj-clmmte-opdtry-639[kmtcd]", "ibghopzs-pibbm-rsdofhasbh-220[sutpe]",
...
-- | parses the given room specification
parse :: String -> Room
parse s =
    let
        -- encrypted room name, with dashes replaced with spaces
        n = foldl1 (\s1 s2 -> s1 ++ " " ++ s2) $ init $ splitOn "-" s
        -- sector ID
        id = read $ filter isDigit $ last $ splitOn "-" s
        -- checksum
        c = filter isLower $ last $ splitOn "-" s
    in
        (n, id, c)

-- | returns checksum of the given encrypted name
checksum :: String -> String
checksum n =
    let
        -- encrypted name without spaces
        n' = filter isLower n
        -- letter frequencies in the encrypted name
        fs = map (\cs -> (head cs, length cs)) $ group $ sort n'
        -- comparison function to sort letters by frequency (highest first),
        -- with ties broken by alphabetic order
        cmp (c1, f1) (c2, f2) =
            case compare f1 f2 of
                LT -> GT
                GT -> LT
                EQ -> compare c1 c2
        -- letters sorted by frequency, with ties broken by alphabetic order
        cs = map fst $ sortBy cmp fs
    in
        take 5 cs

-- | determines whether the given room is real (by verifying the checksum)
real :: Room -> Bool
real (n, _, c) = checksum n == c

-- | solution to part one of the puzzle
solution1 =
    let
        -- parsed room specifications
        rs = map parse rooms
        -- real rooms
        rs' = filter real rs
        -- sum of sector IDs of real rooms
        n = sum $ map (\(_, id, _) -> id) rs'
    in
        n

-- | utility functions to return the index of a letter in the alphabet and vice
-- | versa
alphabet :: [Char]
alphabet =
...
indexForLetter c =
    case elemIndex c alphabet of
        Just n  -> n
        Nothing -> error "Given character is not a letter of the alphabet"

letterForIndex :: Int -> Char
letterForIndex n = alphabet !! n

-- | given an encrypted room name and its sector ID, returns the real name
decrypt :: String -> Int -> String
decrypt n id =
    let
        -- function to rotate a letter forward the given number of times, or do
        -- nothing if given a space
        rot n c =
            if
                c == ' '
            then
                ...
                letterForIndex $ (indexForLetter c + n) `mod` 26
    in
        map (rot id) n
...
solution2 =
    let
        ...
        -- real rooms
        rs' = filter real rs
        -- decrypted real rooms' names with their sector IDs
        rs'' = map (\(n, id, _) -> (decrypt n id, id)) rs'
        -- real names and sector IDs of room(s) related to some North Pole stuff
        rs''' = filter (\(n, _) -> isInfixOf "north" n) rs''
    in
        rs'''
